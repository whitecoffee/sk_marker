...
{
  ros::init(argc, argv, "left_upper_arm");
  ros::NodeHandle n;
  ros::Rate r(30.0);
  ...
  while (ros::ok())
  {
    ...
    try{
      ...
      listener.waitForTransform("tracker/left_shoulder", "tracker/left_elbow", ros::Time(0), ros::Duration(1.0));
      listener.lookupTransform("tracker/left_shoulder", "tracker/left_elbow", ros::Time(0), shoulder_elbow_transform);
    ...
    shoulder.header.frame_id = shoulder_elbow.header.frame_id = "tracker/left_shoulder";
    ...
    shoulder.ns = shoulder_elbow.ns = "left_upper_arm_model";
    shoulder.id = 0;
    shoulder_elbow.id = 1;
