...
{
  ros::init(argc, argv, "left_upper_leg");
  ros::NodeHandle n;
  ros::Rate r(30.0);
  ...
  while (ros::ok())
  {
    ...
    try{
      ...
      listener.waitForTransform("tracker/left_hip", "tracker/left_knee", ros::Time(0), ros::Duration(1.0));
      listener.lookupTransform("tracker/left_hip", "tracker/left_knee", ros::Time(0), hip_knee_transform);
    ...
    hip.header.frame_id = hip_knee.header.frame_id = "tracker/left_hip";
    ...
    hip.ns = hip_knee.ns = "left_upper_leg_model";
    hip.id = 0;
    hip_knee.id = 1;
