...
{
  ros::init(argc, argv, "left_fore_arm");
  ros::NodeHandle n;
  ros::Rate r(30.0);
  ...
  while (ros::ok())
  {
    ...
    try{
      ...
      listener.waitForTransform("tracker/left_elbow", "tracker/left_hand", ros::Time(0), ros::Duration(1.0));
      listener.lookupTransform("tracker/left_elbow", "tracker/left_hand", ros::Time(0), elbow_hand_transform);
    ...
    elbow.header.frame_id = elbow_hand.header.frame_id = "tracker/left_elbow";
    ...
    elbow.ns = elbow_hand.ns = "left_fore_arm_model";
    elbow.id = 0;
    elbow_hand.id = 1;
